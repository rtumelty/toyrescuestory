...
#else
			if ( Input.touches[0].deltaPosition.magnitude > 15f ) GlobalVariables.SCREEN_DRAGGING = _startDrag = true;
#endif
			
			if ( _startDrag && panUnlocked )
			{
#if UNITY_EDITOR
				...
				if ( Camera.main.transform.position.x > MAXIMUM_CAMERA_X ) Camera.main.transform.position = new Vector3 ( MAXIMUM_CAMERA_X, Camera.main.transform.position.y, Camera.main.transform.position.z );
				else if ( Camera.main.transform.position.x < MINIMUM_CAMERA_X ) Camera.main.transform.position = new Vector3 ( MINIMUM_CAMERA_X, Camera.main.transform.position.y, Camera.main.transform.position.z );
				
				if ( Camera.main.transform.position.z > MAXIMUM_CAMERA_Z ) Camera.main.transform.position = new Vector3 ( Camera.main.transform.position.x, Camera.main.transform.position.y, MAXIMUM_CAMERA_Z );
				else if ( Camera.main.transform.position.z < MINIMUM_CAMERA_Z ) Camera.main.transform.position = new Vector3 ( Camera.main.transform.position.x, Camera.main.transform.position.y, MINIMUM_CAMERA_Z );
			}
